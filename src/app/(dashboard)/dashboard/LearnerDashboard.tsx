        ...
        <div className="grid lg:grid-cols-3 gap-6">
                      ...
                      <div key={item.module} className="flex items-center gap-3">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between mb-1">
                            <span className="text-sm font-medium text-slate-700 truncate">{item.module}</span>
                            <span className="text-xs text-slate-400 ml-2 shrink-0">{item.credits} cr · {item.progress}%</span>
                          </div>
                          <Progress value={item.progress} variant={item.progress === 100 ? "success" : "default"} />
                        </div>
                        {item.progress === 100 && (
                ...
                <div className="p-5">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1">
                      <h3 className="text-sm font-semibold text-amber-800 mb-1">Remediation Recommended</h3>
          ...
          <div className="space-y-6">
            <motion.div custom={4} initial="hidden" animate="visible" variants={fadeUp}>
              <Card>
                <div className="p-5">
                  <h2 className="text-base font-semibold font-display text-slate-900 mb-4">Milestones</h2>
                  <div className="space-y-3">
                    {[
                      ...
                      <div key={ms.title} className="flex items-center gap-3">
                        <div className={cn("h-8 w-8 rounded-lg flex items-center justify-center shrink-0", ms.done ? "bg-emerald-100" : "bg-slate-100")}>
                          {ms.done ? <Award className="h-4 w-4 text-emerald-600" /> : <Target className="h-4 w-4 text-slate-400" />}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className={cn("text-sm font-medium", ms.done ? "text-emerald-700" : "text-slate-700")}>{ms.title}</div>
            ...
            <motion.div custom={5} initial="hidden" animate="visible" variants={fadeUp}>
              <Card>
                <div className="p-5">
                  <div className="flex items-center justify-between mb-4">
                    <h2 className="text-base font-semibold font-display text-slate-900">Recent Credentials</h2>
            ...
            <motion.div custom={6} initial="hidden" animate="visible" variants={fadeUp}>
              <Card>
                <div className="p-5">
                  <h2 className="text-base font-semibold font-display text-slate-900 mb-3">Affordability</h2>
                  <div className="space-y-2 text-sm">
                    <div className="flex items-center justify-between"><span className="text-slate-500">Total pathway cost</span><span className="font-semibold text-slate-900">{formatCurrency(mockLearnerStats.total_spent + mockLearnerStats.remaining_cost)}</span></div>
                    <div className="flex items-center justify-between"><span className="text-slate-500">Paid so far</span><span className="font-medium text-slate-700">{formatCurrency(mockLearnerStats.total_spent)}</span></div>
                    <div className="flex items-center justify-between"><span className="text-slate-500">Remaining</span><span className="font-medium text-slate-700">{formatCurrency(mockLearnerStats.remaining_cost)}</span></div>
                    <div className="pt-2 border-t border-slate-100 mt-2">
                      <div className="flex items-center gap-1.5 text-xs text-emerald-600"><Zap className="h-3 w-3" />3 free alternatives available in your pathway</div>
                    </div>
                  </div>
                </div>
              </Card>
