    ...
    <div className="min-h-screen flex">
        ...
        <div className="relative flex flex-col justify-center px-16">
          <Link href="/" className="flex items-center gap-3 mb-12">
            ...
            <div>
              ...
              <div className="text-[10px] font-semibold text-emerald-200 tracking-widest uppercase">South Africa</div>
            </div>
          </Link>
          <h1 className="text-3xl font-bold font-display text-white leading-tight mb-4">Welcome back.<br />Keep building your future.</h1>
          <p className="text-base text-emerald-100/80 max-w-sm">Continue your personalised learning pathway, earn credentials, and connect with real opportunities across South Africa.</p>
        </div>
      </div>
      <div className="flex-1 flex items-center justify-center px-6 py-12 bg-white">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="w-full max-w-[400px]">
          ...
          <p className="text-sm text-slate-500 mb-8">Don&apos;t have an account?{" "}<Link href="/signup" className="text-emerald-600 font-medium hover:underline">Get started free</Link></p>
          ...
          <div className="relative mb-6"><Separator /><span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 bg-white px-3 text-xs text-slate-400">or continue with email</span></div>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <Label htmlFor="email" className="text-slate-700">Email</Label>
              <div className="relative mt-1.5">
                <Mail className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400" />
                <Input id="email" type="email" placeholder="thabo@example.co.za" className="pl-10" required value={email} onChange={(e) => setEmail(e.target.value)} />
              </div>
            </div>
            <div>
            ...
            <Button type="submit" className="w-full h-11" isLoading={loading}>Sign In<ArrowRight className="h-4 w-4 ml-2" /></Button>
          </form>
        </motion.div>
      </div>
    </div>
